...
const oldHardcodedTags: string[] = ['MLOps', 'GenAI', 'Python', 'CI/CD', 'Data', 'Edge ML', 'AI'];
const foundTags = matches.map((m: RegExpMatchArray) => m[1].trim());
...
const hasOldTags = foundTags
  .slice(0, 7)
  .every((tag: string, i: number) => tag === oldHardcodedTags[i]);
